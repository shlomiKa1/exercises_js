...
// Exercise 9
numbers = [10, 20, 30];
const sumNums = (nums) => nums.reduce((s, v) => s + v, 0);

// Exercise 10
const multNums = (nums) => nums.reduce((m, v) => m * v);
...
// Exercise 39
function arrC(arr) {
  const check = arr.reduce((newArr, val) => {
    if (newArr.includes(val)) return false;
